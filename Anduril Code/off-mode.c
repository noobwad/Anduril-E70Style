...
uint8_t off_state(Event event, uint16_t arg) {
    ...
    #endif
    #if (B_TIMING_ON == B_PRESS_T) //original: B_TIMING_ON == B_PRESS_T
    // hold (initially): go to lowest level (floor), but allow abort for regular click
    else if (event == EV_click2_press) { //original: EV_click1_press
        set_level(nearest_level(1));
        return MISCHIEF_MANAGED;
    }
    #endif  // B_TIMING_ON == B_PRESS_T
    // hold: go to lowest level
    else if (event == EV_click2_hold) { //original: EV_click1_hold
    ...
    else if (event == EV_click2_hold_release) { //original: EV_click1_hold_release
        set_state(steady_state, 1);
        return MISCHIEF_MANAGED;
    }
    ...
    else if (event == EV_2clicks) {//original EV_1click
        #if (B_TIMING_ON != B_TIMEOUT_T)
    ...
    // click, hold: momentary at ceiling or turbo
    else if (event == EV_click3_hold) { //origianl click2_hold
     ...
     //3 clicks: highest mode (ceiling) (originally 2 clicks)
    ...
    else if (event == EV_5clicks) { //originally 3 clicks
    ...
    else if (event == EV_click5_hold) { //originally 3 clicks and hold
  ...
  /*  // 5 clicks: momentary mode (!!removed from this version)
